...
const fileNameSlips = 'slips.json'
const fileNamePick = 'paging.json'
const dataLocation = path.resolve(__dirname, '..','data', fileName);
const dataLocationSlips = path.resolve(__dirname, '..','data', fileNameSlips);
const dataLocationPaging = path.resolve(__dirname, '..','data', fileNamePick);
...
export default class AddPaging extends Component {
    ...
    componentDidMount = async () => {
        ...
        const paging = await Load.loadFromFile(dataLocationPaging)
        this.setState({
            add: results,
            pick: paging,
        }, () => {
    ...
    addPick = (values) => { 
        ...
        this.setState(prevState => ({
            pick: [...prevState.pick, values]
      }), () => {
            Updates.writeToFile(dataLocation, this.state.pick)
            Alerts.success(`Added ${values.barcode} to paging list`)
      })
    }
    ...
    removePick = (values, index) => {
        this.setState((prevState) => ({
            pick: prevState.pick.filter((_, i) => i != index)
        }), () => {
            Updates.writeToFile(dataLocation, this.state.pick)
            Alerts.info(`Removed ${values.barcode} from paging list`)
        })
    }
    ...
    handleOptions = (e, option) => {
        ...
        switch(option){
            ...
            break;
            case "addAll":
                this.handlePickUpdate(list)
                this.handleAddUpdate({})
                Alerts.success('Added all items to paging list')
            break;
            case "removeAll":
                this.handleAddUpdate({})
    ...
    handlePickUpdate = (items) => {
        this.setState(prevState => ({
            pick: [...prevState.pick, ...items]
        }), () => {
            Updates.writeToFile(dataLocation, this.state.pick)
        })
    }
    ...
    render(){
        return(
            <div className="btn-group-vertical" style={{marginLeft: '10px'}}>
                <button className="btn btn-success option-button" style={{marginBottom: '20px'}} onClick={(e) => this.props.getPagingSlips(e, 'morning')}>Morning Slips</button>
                <button className="btn btn-success option-button" style={{marginBottom: '20px'}} onClick={(e) => this.props.getPagingSlips(e, 'evening')}>Evening Slips</button>
            ...
            <table className="table">
                ...
                {Object.keys(this.props.data).map((items, index) => {
                    ...
                    <tr key={index}>
                        ...
                        {this.props.data[items].shelf && this.props.data[items].shelf.shelf_number ?
